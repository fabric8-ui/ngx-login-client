...
# Build builder image
docker build -t ngx-login-client-builder -f Dockerfile.builder .
mkdir -p dist && docker run --detach=true --name=ngx-login-client-builder -e "FABRIC8_WIT_API_URL=http://api.openshift.io/api/" -e JENKINS_URL -e GIT_BRANCH -e "CI=true" -e GH_TOKEN -e NPM_TOKEN -t -v $(pwd)/dist:/dist:Z ngx-login-client-builder
...
## Build prod
docker exec ngx-login-client-builder npm run build

## Exec functional tests
docker exec ngx-login-client-builder ./run_unit_tests.sh
...
## Exec functional tests
docker exec ngx-login-client-builder ./run_functional_tests.sh
